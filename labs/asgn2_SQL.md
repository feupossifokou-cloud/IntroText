...
SELECT COUNT(*) AS assignments_2024
FROM assignments
WHERE due_date >= '2024-01-01'
  AND due_date <  '2025-01-01';


1.- Write a query to list all courses, concatenating the `course_id` and `course_name` fields with a colon and a space between them. Like this:   
   "COMP1151: IT Essentials"


   SELECT 
    course_id || ': ' || course_name AS course_info
FROM courses;
